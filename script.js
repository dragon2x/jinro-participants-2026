...
function addTeacher() {
  ...
  row.innerHTML = `
    ...
    <div>
      <span class="sub-label">교원명 <span class="required">*</span></span>
      <input type="text" class="field-input t-name" placeholder="홍길동" maxlength="20" />
      <span class="error-msg t-name-err"></span>
    </div>
    <div>
...
function validate() {
  ...
  const rows = document.querySelectorAll('.teacher-row');
  if (rows.length === 0) {
    showError('err-teachers', '교원을 1명 이상 입력해 주세요.');
    valid = false;
  } else {
    clearError('err-teachers');
  }
  ...
  rows.forEach((row) => {
    ...
    const phoneEl = row.querySelector('.t-phone');
    const nameErr  = row.querySelector('.t-name-err');
    const idErr    = row.querySelector('.t-id-err');
    const phoneErr = row.querySelector('.t-phone-err');

    if (!nameEl.value.trim()) {
      nameErr.textContent = '교원명을 입력해 주세요.';
    ...
    if (!phoneEl.value.trim()) {
      phoneErr.textContent = '연락처를 입력해 주세요.';
      phoneEl.classList.add('error');
      valid = false;
    } else if (!/^\d{3}-\d{3,4}-\d{4}$/.test(phoneEl.value)) {
      phoneErr.textContent = '연락처 형식을 확인해 주세요.';
      phoneEl.classList.add('error');
      valid = false;
    } else {
